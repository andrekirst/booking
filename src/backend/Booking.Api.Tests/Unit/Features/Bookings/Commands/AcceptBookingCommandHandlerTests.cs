...
public class AcceptBookingCommandHandlerTests
{
    ...
    private readonly AcceptBookingCommandHandler _handler;

    public AcceptBookingCommandHandlerTests()
    {
        _repository = Substitute.For<IEventSourcedRepository<BookingAggregate>>();
        _logger = Substitute.For<ILogger<AcceptBookingCommandHandler>>();
        _handler = new AcceptBookingCommandHandler(_repository, _logger);
    }

    [Fact]
    public async Task Handle_WithValidBookingId_ShouldAcceptBookingAndReturnTrue()
    {
        // Arrange
        var bookingId = Guid.NewGuid();
        var command = new AcceptBookingCommand(bookingId);
