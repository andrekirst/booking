...
public class BookingRejectedEventHandlerTests
{
    private readonly IProjectionService<BookingAggregate, BookingReadModel> _projectionService;
    private readonly ILogger<BookingRejectedEventHandler> _logger;
    private readonly BookingRejectedEventHandler _handler;

    public BookingRejectedEventHandlerTests()
    {
        _projectionService = Substitute.For<IProjectionService<BookingAggregate, BookingReadModel>>();
        _logger = Substitute.For<ILogger<BookingRejectedEventHandler>>();
        _handler = new BookingRejectedEventHandler(_projectionService, _logger);
    }
    ...
    public async Task Handle_ShouldCallProjectionService_WithCorrectBookingId()
    {
        ...
        var bookingEvent = new BookingRejectedEvent
        {
            BookingId = bookingId,
