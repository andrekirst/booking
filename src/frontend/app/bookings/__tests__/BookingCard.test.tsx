...
describe('BookingCard', () => {
  ...
  describe('Admin Functions', () => {
    ...
    it('should show admin buttons for pending booking when user is administrator', () => {
      const pendingBooking = { ...mockBooking, status: BookingStatus.Pending };
      render(
        <BookingCard 
      ...
      expect(screen.getByText('Admin-Aktionen')).toBeInTheDocument();
    });

    it('should not show admin buttons for non-pending booking even when user is administrator', () => {
    ...
    it('should not show admin buttons when user is not administrator', () => {
      const pendingBooking = { ...mockBooking, status: BookingStatus.Pending };
      render(
        <BookingCard 
          booking={pendingBooking} 
    ...
    it('should call onAccept when accept button is clicked', () => {
      ...
      expect(mockOnAccept).toHaveBeenCalledWith(pendingBooking.id);
    ...
    it('should call onReject when reject button is clicked', () => {
      ...
      expect(mockOnReject).toHaveBeenCalledWith(pendingBooking.id);
      expect(mockOnClick).not.toHaveBeenCalled(); // Should not trigger card click
    });

    it('should have proper styling for admin buttons', () => {
      const pendingBooking = { ...mockBooking, status: BookingStatus.Pending };
      render(
        <BookingCard 
          booking={pendingBooking} 
      ...
      expect(acceptButton).toHaveClass('bg-emerald-100', 'text-emerald-800');
      expect(rejectButton).toHaveClass('bg-rose-100', 'text-rose-800');
    });
  });
});